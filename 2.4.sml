...
fun min (a,b):int=if a>b then b else a;
...
fun skyline(l:int array,line:int array,pos:int,n:int)=
    ...
    in
        (renew(line,left,height,right); skyline(l:int array,line:int array,pos+3:int,n:int) )
    end;

fun printans(l:int array,pos:int,exval:int,len:int):unit=
